...
// Same wrong-path guess as anonymous-analysis.factory.ts and
// lawyer-inquiry.factory.ts's identical bug, both already fixed against
// the same confirmed real source (document.factory.ts's own working
// Amendment #15 import). This file's own header comment already
// predicted this exact fix ("if that import is wrong, both factories
// break the same way and need the same one-line fix") — now a third.
