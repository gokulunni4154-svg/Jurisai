...
// AMENDMENT (File 170) — adds createSignedDownloadUrl(), mirroring
// DocumentRepository's identical Amendment #12 method exactly, targeting
// the legal-vault-exports bucket (File 162's migration) instead of
// legal-vault-documents. Deliberately pure data-layer, same division of
// labor as DocumentRepository's version: this method does NOT check
// whether storagePath belongs to a row the caller is allowed to see, or
// whether that row is 'completed' — that's PdfExportService#getDownloadUrl's
// job (File 171), which must call getPdfExportById() and check status
...
/**
 * Must match the Storage bucket name created by File 162's migration
 * exactly. Duplicated here rather than imported from anywhere else,
 * same "flagged duplication, not silently synced" tradeoff
 * DOCUMENTS_BUCKET already accepts in document.repository.ts.
 */
const EXPORTS_BUCKET = 'legal-vault-exports';
...
export class PdfExportRepository extends BaseRepository<'pdf_exports'> {
  ...
  /**
   * AMENDMENT (File 170) — new. Generates a time-limited signed URL for
   * downloading a completed export's PDF from Storage. Deliberately pure
   * data-layer, same division of labor as
   * DocumentRepository#createSignedDownloadUrl (Amendment #12): this
   * method does NOT check whether `storagePath` belongs to a row the
   * current actor is allowed to see, nor whether that row is
   * 'completed' — that's PdfExportService#getDownloadUrl's job (File
   * 171), which must call getPdfExportById() (RLS-scoped) and check
   * status BEFORE ever calling this method. Calling this directly with
   * an arbitrary storagePath, bypassing that check, would defeat this
   * table's RLS-based visibility model for the one operation that
   * doesn't go through a `.from('pdf_exports')` query and therefore
   * isn't RLS-scoped at all — Storage signed URLs are generated by the
   * Storage API, not filtered by Postgres RLS on the `pdf_exports`
   * table.
   *
   * `expiresInSeconds` defaults to 300 (5 minutes) — same default and
   * same reasoning as DocumentRepository#createSignedDownloadUrl: long
   * enough for a real download to start on a slow connection, short
   * enough that a leaked/logged URL has a small blast radius.
   */
  async createSignedDownloadUrl(
    ...
    if (error || !data) {
      throw new DatabaseError(
        'Failed to create signed download URL for pdf export',
        error ?? new Error('Storage returned no data for signed URL request'),
        { bucket: EXPORTS_BUCKET, storagePath },
      );
    }
